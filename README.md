...
heap 은 이진트리에서 특정한 조건을 이룬 구조입니다.

heap은 부분 순서 트리(Partial order tree)이기도 한데, 이는 모든 노드가 자식 노드보다 값이 크거나 같은 트리입니다.

부모가 자식의 키 값보다 작은 최대 heap (max heap), 부모가 자식 노드보다 작은 최소 heap (min heap) 두 종류가 존재합니다.

heap은 보통 3가지 부분으로 나누어 구현합니다. 보통 구조체 선언, heap 삽입과정, heap 삭제과정으로 구현됩니다.

새로운 노드를 삽입하여 부모노드와 비교하여 부모노드보다 크면 위치를 바꾸는 heap 삽입과정은 InsertHeap으로 구현했고, 부모노드 자식노드를 비교한 후
삭제하는 heap 삭제과정은 DeleteHeap으로 구현했습니다.
